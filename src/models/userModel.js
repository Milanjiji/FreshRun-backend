...
const findById = async (id) => {
  const result = await db.query(
    'SELECT id, firebase_uid, phone, role, full_name, email, house_number, address_line, landmark, pincode, city, delivery_message, current_address_id, is_profile_complete, is_active, created_at FROM users WHERE id = $1',
    [id]
  );
...
const findByFirebaseUid = async (firebaseUid) => {
  const result = await db.query(
    'SELECT id, firebase_uid, phone, role, full_name, email, house_number, address_line, landmark, pincode, city, delivery_message, current_address_id, is_profile_complete, is_active, created_at FROM users WHERE firebase_uid = $1',
    [firebaseUid]
  );
  return result.rows[0];
};
...
const createUser = async (id, firebaseUid, phone, role) => {
  const result = await db.query(
    'INSERT INTO users (id, firebase_uid, phone, role, is_active) VALUES ($1, $2, $3, $4, $5) RETURNING id, firebase_uid, phone, role, full_name, email, house_number, address_line, landmark, pincode, city, delivery_message, current_address_id, is_profile_complete, is_active, created_at',
